var textStrip = ['0', '4', '5', '0', '1', '0', '1', '2', '2', '3', '4', '9', '3'];
...
function drawStrip(context, x, y) {
    for (var k = 0; k <= 20; k++) {
...
function drawMatrixText() {
    var canvas = document.getElementById('matrixText');
    if (!canvas) return;
    ...
    for (var j = 0; j < stripCount; j++) {
        ...
        if (stripY[j] > 1358) {
            ...
            drawStrip(context, stripX[j], stripY[j]);
        } else drawStrip(context, stripX[j], stripY[j]);
        
        stripY[j] += dY[j];
    }

    setTimeout(drawMatrixText, 70);
}
